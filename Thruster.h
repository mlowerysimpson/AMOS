...
#define LEFT_THRUST_PIN 41  //gpio pin to use for PWM for left thruster (requires Alt0 alternative pin function)
#define RIGHT_THRUST_PIN 40 //gpio pin to use for PWM for right thruster (requires Alt0 alternative pin function)

#define REVERSE_THRUST 1 //define this if you get the wiring backwards on the props

class Thruster {
public:
	Thruster(bool bLeftThruster, bool bRightThruster, bool bAirThruster);//thruster constructor
	~Thruster();
	...
	void Stop();//send a "stopped signal" to both thrusters
	...
	float GetAirRudderAngle();//get the angle of the air rudder (if available) in degrees
	...
	void SetAirPropSpeedAndRudderAngle(float fAirSpeed, float fAirRudderAngle);//set speed of air propeller and the angle of the air rudder
	static void OneTimeSetup();//setup procedure to be performed once per power-cycle of the T200 thrusters
